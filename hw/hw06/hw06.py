class VendingMachine:
    ...
    'Here is your soda.'
    """
    def __init__(self, name, price, count=0, input=0):
        self.name = name
        self.price = price
        self.count = count
        self.input = input
    ...
    def vend(self):
        if self.count == 0:
            ...
            difference = self.price - self.input
            return f'You must add ${difference} more funds.'
        elif self.input > self.price:
            self.count -= 1
            change = self.input - self.price
            self.input = 0
    ...
    def restock(self, more):
        self.count += more
        return f'Current {self.name} stock: {self.count}'
...
def store_digits(n):
    ...
    if n < 10: 
        ...
        while first > 10:
            first //= 10
            power += 1
        return Link(first, store_digits(n - (first*(10 ** power))))
...
def two_list(vals, amounts):
    ...
    if len(vals) == 1:
        ...
        link.rest = two_list(vals[1:], amounts[1:])
    else:
        link.first, amounts[0] = vals[0], amounts[0] - 1
        link.rest = two_list(vals, amounts)
    return link
