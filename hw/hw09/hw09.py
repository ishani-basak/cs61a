...
def cs_classes(post):
    ...
    True
    """
    return bool(re.search(r"[CS|cs|Cs|cS]\s*\d+[ABC]?", post))
...
def match_time(text):
    ...
    ['2:00']
    """
    return re.findall(r"((?:[0-1]?\d|2[0-3]):[0-5]\d(?:[A|P]M)?)", text)
...
def area_codes(text):
    ...
    []
    """
    return re.findall(r"\(?(\d\d\d)\)?\s?-?\d\d\d\s?-?\d\d\d\d\b", text)
...
def most_common_code(text):
    ...
    for code in area_codes(text):
        if codes.count(code) > max_count:
            max_code = code
            max_count = codes.count(code)
    return max_code