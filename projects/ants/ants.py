...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        place = self.place
        distance = 0
        while ((len(place.bees) == 0) or self.min_range > distance or self.max_range < distance) and (not place.is_hive):  
            distance += 1
...
class HungryAnt(Ant):
    ...
    def action(self, gamestate):
        if self.chew_countdown > 0:
            ...
            near_bee = random_bee(self.place.bees)
            if near_bee != None:
                near_bee.reduce_health(near_bee.health)
...
class GameState:
    ...
    bee_xentrances -- A list of places that bees can enter
    """
