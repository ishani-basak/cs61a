...
def announce_highest(who, last_score=0, running_high=0):
    ...
    # BEGIN PROBLEM 7
    def say(score0, score1):
        new_highest = running_high
        if who==0:
            if score0 - last_score > running_high:
                new_highest = score0 - last_score
                print(new_highest, "point(s)! That's a record gain for Player 0!")
            return announce_highest(who, score0, new_highest)
        else:
            if score1 - last_score > running_high:
                new_highest = score1 - last_score
                print(new_highest, "point(s)! That's a record gain for Player 1!")
            return announce_highest(who, score1, new_highest)
    return say
    # END PROBLEM 7
...
def max_scoring_num_rolls(dice=six_sided, trials_count=1000):
    ...
    """
    # BEGIN PROBLEM 9
    max_roll, max_avg, rolls = 0, 0, 1
    current_avg = make_averaged(roll_dice, trials_count)
    while rolls <= 10: 
        avg = current_avg(rolls, dice)
        if avg > max_avg:
            max_avg = avg
            max_roll = rolls
        rolls += 1
...
def hog_pile_strategy(score, opponent_score, cutoff=8, num_rolls=6):
    ...
    """
    # BEGIN PROBLEM 11
    score += picky_piggy(opponent_score) + hog_pile(score, opponent_score)
    if picky_piggy(opponent_score) >= cutoff:
        return 0
    elif hog_pile(score, opponent_score) == score:
        return 0
    else:
        return num_rolls
    # END PROBLEM 11
