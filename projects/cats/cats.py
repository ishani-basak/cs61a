...
def choose(paragraphs, select, k):
    ...
    """
    # BEGIN PROBLEM 1
    true_paragraphs = [p for p in paragraphs if select(p)]
    if k >= len(true_paragraphs):
        return ''
    else:
...
def about(topic):
    ...
    def select(p):
        for w in topic:
            if w in split(remove_punctuation(lower(p))):
                return True
        return False
    return select
    # END PROBLEM 2
...
def accuracy(typed, reference):
    ...
    if t == r == 0:
        ...
        if t > r:
            correct -= (t - r)
        for w in range(min(r, t)):
            if typed_words[w] != reference_words[w]:
                correct -= 1
...
def autocorrect(typed_word, valid_words, diff_function, limit):
    ...
    if typed_word in valid_words:
        return typed_word
    min_word = min(valid_words, key=lambda x: diff_function(typed_word, x, limit))
...
def feline_flips(start, goal, limit):
    ...
    """
    # BEGIN PROBLEM 6
    def count_change(start, goal, total):
        if total > limit:
            return total
        elif start == '' or goal == '':
            ...
            total += 1
        return count_change(start[1:], goal[1:], total)
    return count_change(start, goal, 0)
    # END PROBLEM 6
...
def report_progress(sofar, prompt, user_id, upload):
    ...
    id_dict = {'id': user_id, 'progress': count_correct/len(prompt)}
    upload(id_dict)
    return id_dict['progress']
    # END PROBLEM 8
...
def time_per_word(words, times_per_player):
    ...
    for i in range(len(times_per_player)):
        new_times = []
        new_times += [[times_per_player[i][j]-times_per_player[i][j-1] for j in range(1, len(times_per_player[i]))]]
        match_times += new_times
    return match(words, match_times)
    # END PROBLEM 9
...
def fastest_words(match):
    ...
    for w in word_indices:
        f_time = time(match, 0, w)
        f_player = 0
        for i in player_indices:
            if time(match, i, w) < f_time:
                f_time = time(match, i, w)
                f_player = i
        f_words[f_player] += [word_at(match, w)]
    return f_words
    # END PROBLEM 10
