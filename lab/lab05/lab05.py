...
def closer_city(lat, lon, city_a, city_b):
    ...
    'Bucharest'
    """
    current = make_city('Current City', lat, lon)
    if distance(current, city_a) < distance(current, city_b):
...
def sprout_leaves(t, leaves):
    ...
    """
    if is_leaf(t):
        return tree(label(t), [tree(leaves[i]) for i in range(len(leaves))])
